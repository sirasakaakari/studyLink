...
// ↓これを追加
Route::get('/fix-duplicate-wordbooks', function () {
    $wordbooks = App\Models\Wordbook::whereNull('user_id')->get();
    foreach ($wordbooks as $wb) {
        App\Models\Word::where('wordbook_id', $wb->id)->delete();
        $wb->delete();
    }
    $wordbook = App\Models\Wordbook::create(['name' => '英検2級', 'user_id' => null]);
    $words = [
    ...
    foreach ($words as $w) {
        App\Models\Word::create(['word' => $w['word'], 'meaning' => $w['meaning'], 'wordbook_id' => $wordbook->id]);
    }
    return 'Done! wordbook_id: ' . $wordbook->id;
});
//
